...
func ConvertMapToValuesForCreate(stmt *germ.Statement, mapValue map[string]interface{}) (values clause.Values) {
...
func ConvertSliceOfMapToValuesForCreate(stmt *germ.Statement, mapValues []map[string]interface{}) (values clause.Values) {
...
func hasReturning(tx *germ.DB, supportReturning bool) (bool, germ.ScanMode) {
	if supportReturning {
		if c, ok := tx.Statement.Clauses["RETURNING"]; ok {
			...
			return true, germ.ScanUpdate
		}
	}
	return false, 0
}

func checkMissingWhereConditions(db *germ.DB) {
	if !db.AllowGlobalUpdate && db.Error == nil {
		...
		if !withCondition {
			db.AddError(germ.ErrMissingWhereClause)
		}
		return
	}
}
