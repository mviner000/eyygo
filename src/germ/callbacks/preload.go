...
//
//nolint:cyclop
func preloadEntryPoint(db *germ.DB, joins []string, relationships *schema.Relationships, preloads map[string][]interface{}, associationsConds []interface{}) error {
	...
	for _, name := range preloadNames {
		if relations := relationships.EmbeddedRelations[name]; relations != nil {
			...
			if joined, nestedJoins := isJoined(name); joined {
				...
				default:
					return germ.ErrInvalidData
				}
			} else {
				tx := db.Table("").Session(&germ.Session{Context: db.Statement.Context, SkipHooks: db.Statement.SkipHooks})
			...
			return fmt.Errorf("%s: %w for schema %s", name, germ.ErrUnsupportedRelation, db.Statement.Schema.Name)
		}
	}
	return nil
}

func preloadDB(db *germ.DB, reflectValue reflect.Value, dest interface{}) *germ.DB {
	tx := db.Session(&germ.Session{Context: db.Statement.Context, NewDB: true, SkipHooks: db.Statement.SkipHooks, Initialized: true})
...
func preload(tx *germ.DB, rel *schema.Relationship, conds []interface{}, preloads map[string][]interface{}) error {
	var (
	...
	if len(values) != 0 {
		for _, cond := range conds {
			if fc, ok := cond.(func(*germ.DB) *germ.DB); ok {
				tx = fc(tx)
			} else {
