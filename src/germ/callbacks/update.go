...
func SetupUpdateReflectValue(db *germ.DB) {
...
// Update update hook
func Update(config *Config) func(db *germ.DB) {
...
func ConvertToAssignments(stmt *germ.Statement) (set clause.Set) {
	var (
	...
	default:
		...
		if !updatingValue.CanAddr() || stmt.Dest != stmt.Model {
			// different schema
			updatingStmt := &germ.Statement{DB: stmt.DB}
