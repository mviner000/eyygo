...
func BeforeCreate(db *germ.DB) {
	if db.Error == nil && db.Statement.Schema != nil && !db.Statement.SkipHooks && (db.Statement.Schema.BeforeSave || db.Statement.Schema.BeforeCreate) {
		callMethod(db, func(value interface{}, tx *germ.DB) (called bool) {
...
// Create create hook
func Create(config *Config) func(db *germ.DB) {
		...
		if ok {
			if c, ok := db.Statement.Clauses["ON CONFLICT"]; ok {
				if onConflict, _ := c.Expression.(clause.OnConflict); onConflict.DoNothing {
					mode |= germ.ScanOnConflictDoNothing
				}
			}
...
func ConvertToCreateValues(stmt *germ.Statement) (values clause.Values) {
	...
	default:
		var (
			...
			_, updateTrackTime        = stmt.Get("germ:update_track_time")
			isZero                    bool
		)
		stmt.Settings.Delete("germ:update_track_time")
		...
		case reflect.Slice, reflect.Array:
			...
			if rValLen == 0 {
				stmt.AddError(germ.ErrEmptySlice)
				return
			}
			...
			for i := 0; i < rValLen; i++ {
				...
				if !rv.IsValid() {
					stmt.AddError(fmt.Errorf("slice data #%v is invalid: %w", i, germ.ErrInvalidData))
					return
				}
		...
		default:
			stmt.AddError(germ.ErrInvalidData)
		}
	}
