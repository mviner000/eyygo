				...
				if db.Statement.Schema != nil {
					...
					if isRelations {
						genJoinClause := func(joinType clause.JoinType, parentTableName string, relation *schema.Relationship) clause.Join {
							...
							{
								onStmt := germ.Statement{Table: tableAliasName, DB: db, Clauses: map[string]clause.Clause{}}
...
func Preload(db *germ.DB) {
	if db.Error == nil && len(db.Statement.Preloads) > 0 {
		if db.Statement.Schema == nil {
			db.AddError(fmt.Errorf("%w when using preload", germ.ErrModelValueRequired))
			return
		}
