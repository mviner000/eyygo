...
func SaveBeforeAssociations(create bool) func(db *germ.DB) {
	return func(db *germ.DB) {
...
func SaveAfterAssociations(create bool) func(db *germ.DB) {
	return func(db *germ.DB) {
		if db.Error == nil && db.Statement.Schema != nil {
			selectColumns, restricted := db.Statement.SelectAndOmitColumns(create, !create)
			...
			for _, rel := range db.Statement.Schema.Relationships.Many2Many {
				...
				if joins.Len() > 0 {
					db.AddError(db.Session(&germ.Session{NewDB: true}).Clauses(clause.OnConflict{DoNothing: true}).Session(&germ.Session{
...
func onConflictOption(stmt *germ.Statement, s *schema.Schema, defaultUpdatingColumns []string) (onConflict clause.OnConflict) {
...
func saveAssociations(db *germ.DB, rel *schema.Relationship, rValues reflect.Value, selectColumns map[string]bool, restricted bool, defaultUpdatingColumns []string) error {
	...
	tx := db.Session(&germ.Session{NewDB: true}).Clauses(onConflict).Session(&germ.Session{
	...
	if tx.Statement.FullSaveAssociations {
		tx = tx.Set("germ:update_track_time", true)
	}
...
var visitMapStoreKey = "germ:saved_association_map"

func checkAssociationsSaved(db *germ.DB, values reflect.Value) bool {
