...
func DeleteBeforeAssociations(db *germ.DB) {
	if db.Error == nil && db.Statement.Schema != nil {
		...
		for column, v := range selectColumns {
			...
			case schema.HasOne, schema.HasMany:
				...
				tx := db.Session(&germ.Session{NewDB: true}).Model(modelValue)
			...
			case schema.Many2Many:
				var (
					...
					tx             = db.Session(&germ.Session{NewDB: true}).Model(modelValue).Table(table)
				)
...
func Delete(config *Config) func(db *germ.DB) {
