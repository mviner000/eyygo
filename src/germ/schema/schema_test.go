...
func TestParseSchemaWithMap(t *testing.T) {
	type User struct {
		tests.User
		Attrs map[string]string `germ:"type:Map(String,String);"`
	}
...
func checkUserSchema(t *testing.T, user *schema.Schema) {
	...
	// check fields
	fields := []schema.Field{
		{Name: "ID", DBName: "id", BindNames: []string{"Model", "ID"}, DataType: schema.Uint, PrimaryKey: true, Tag: `germ:"primarykey"`, TagSettings: map[string]string{"PRIMARYKEY": "PRIMARYKEY"}, Size: 64, HasDefaultValue: true, AutoIncrement: true},
		...
		{Name: "DeletedAt", DBName: "deleted_at", BindNames: []string{"Model", "DeletedAt"}, Tag: `germ:"index"`, DataType: schema.Time},
			...
			JoinTable: JoinTable{Name: "UserSpeak", Table: "user_speaks", Fields: []schema.Field{
				{
					...
					Tag: `germ:"primarykey"`, Creatable: true, Updatable: true, Readable: true, PrimaryKey: true, Size: 64,
				},
				{
					Name: "LanguageCode", DBName: "language_code", BindNames: []string{"LanguageCode"}, DataType: schema.String,
					Tag: `germ:"primarykey"`, Creatable: true, Updatable: true, Readable: true, PrimaryKey: true,
				},
			}},
...
func TestEmbeddedStruct(t *testing.T) {
	...
	type Corp struct {
		CorpBase
		Base Company `germ:"embedded;embeddedPrefix:company_"`
	}
...
func TestCompositePrimaryKeyWithAutoIncrement(t *testing.T) {
	type Product struct {
		ProductID    uint `germ:"primaryKey;autoIncrement"`
		LanguageCode uint `germ:"primaryKey"`
		Code         string
		Name         string
	}
	type ProductNonAutoIncrement struct {
		ProductID    uint `germ:"primaryKey;autoIncrement:false"`
		LanguageCode uint `germ:"primaryKey"`
		Code         string
		Name         string
	}
