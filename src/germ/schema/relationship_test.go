...
func TestBelongsToOverrideForeignKey(t *testing.T) {
	...
	type User struct {
		germ.Model
		Profile      Profile `germ:"ForeignKey:ProfileRefer"`
		ProfileRefer int
	}
...
func TestBelongsToOverrideReferences(t *testing.T) {
	...
	type User struct {
		germ.Model
		Profile   Profile `germ:"ForeignKey:ProfileID;References:Refer"`
		ProfileID int
	}
...
func TestBelongsToWithOnlyReferences(t *testing.T) {
	...
	type User struct {
		germ.Model
		Profile      Profile `germ:"References:Refer"`
		ProfileRefer int
	}
...
func TestSelfReferentialBelongsToOverrideReferences(t *testing.T) {
	type User struct {
		...
		Name      string
		CreatedBy *int32
		Creator   *User `germ:"foreignKey:CreatedBy;references:ID"`
	}
...
func TestHasOneOverrideForeignKey(t *testing.T) {
	...
	type User struct {
		germ.Model
		Profile Profile `germ:"ForeignKey:UserRefer"`
	}
...
func TestHasOneOverrideReferences(t *testing.T) {
	...
	type User struct {
		germ.Model
		Refer   string
		Profile Profile `germ:"ForeignKey:UserID;References:Refer"`
	}
...
func TestHasOneOverrideReferences2(t *testing.T) {
	...
	type User struct {
		germ.Model
		ProfileID uint     `germ:"column:profile_id"`
		Profile   *Profile `germ:"foreignKey:ID;references:ProfileID"`
	}
...
func TestMany2ManyOverrideForeignKeyAndReferences(t *testing.T) {
	...
	type User struct {
		germ.Model
		Profiles  []Profile `germ:"many2many:user_profiles;ForeignKey:Refer;JoinForeignKey:UserReferID;References:UserRefer;JoinReferences:ProfileRefer"`
		Profiles2 []Profile `germ:"many2many:user_profiles2;ForeignKey:refer;JoinForeignKey:user_refer_id;References:user_refer;JoinReferences:profile_refer"`
		Refer     uint
	}
...
func TestMany2ManyOverrideForeignKey(t *testing.T) {
	...
	type User struct {
		germ.Model
		Profiles []Profile `germ:"many2many:user_profiles;ForeignKey:Refer;References:UserRefer"`
		Refer    uint
	}
...
func TestMany2ManySharedForeignKey(t *testing.T) {
	...
	type User struct {
		germ.Model
		Profiles []Profile `germ:"many2many:user_profiles;foreignKey:Refer,Kind;joinForeignKey:UserRefer,Kind;References:ProfileRefer,Kind;joinReferences:ProfileR,Kind"`
		Kind     string
		Refer    uint
	}
...
func TestMany2ManyOverrideJoinForeignKey(t *testing.T) {
	...
	type User struct {
		germ.Model
		Profiles []Profile `germ:"many2many:user_profile;JoinForeignKey:UserReferID;JoinReferences:ProfileRefer"`
		Refer    uint
	}
...
func TestMany2ManyWithMultiPrimaryKeys(t *testing.T) {
	type Tag struct {
		ID     uint   `germ:"primary_key"`
		Locale string `germ:"primary_key"`
		Value  string
	}
	...
	type Blog struct {
		...
		Subject    string
		Body       string
		Tags       []Tag `germ:"many2many:blog_tags;"`
		SharedTags []Tag `germ:"many2many:shared_blog_tags;ForeignKey:id;References:id"`
		LocaleTags []Tag `germ:"many2many:locale_blog_tags;ForeignKey:id,locale;References:id"`
	}
...
func TestMultipleMany2Many(t *testing.T) {
	...
	type Person struct {
		ID       int
		Likes    []Thing `germ:"many2many:likes"`
		Dislikes []Thing `germ:"many2many:dislikes"`
	}
...
func TestSelfReferentialMany2Many(t *testing.T) {
	type User struct {
		...
		Name       string
		CreatedBy  int32
		Creators   []User      `germ:"foreignKey:CreatedBy"`
		AnotherPro interface{} `germ:"-"`
	}
...
func TestEmbeddedHas(t *testing.T) {
	...
	type User struct {
		...
		} `germ:"embedded;embeddedPrefix:cat_"`
		Dog struct {
			ID     int
			Name   string
			UserID int
...
func TestPolymorphic(t *testing.T) {
	...
	t.Run("has one with custom polymorphic type and id", func(t *testing.T) {
		...
		type Cat struct {
			ID   int
			Name string
			Toy  Toy `germ:"polymorphic:Owner;polymorphicType:Type;polymorphicId:RefId"`
		}
	...
	t.Run("has one with only polymorphic type", func(t *testing.T) {
		...
		type Cat struct {
			ID   int
			Name string
			Toy  Toy `germ:"polymorphic:Owner;polymorphicType:Type"`
		}
	...
	t.Run("has many with custom polymorphic type and id", func(t *testing.T) {
		...
		type Cat struct {
			ID   int
			Name string
			Toys []Toy `germ:"polymorphicType:Type;polymorphicId:RefId"`
		}
...
func TestEmbeddedBelongsTo(t *testing.T) {
	...
	type Org struct {
		ID              int
		PostalAddress   Address `germ:"embedded;embeddedPrefix:postal_address_"`
		VisitingAddress Address `germ:"embedded;embeddedPrefix:visiting_address_"`
		AddressID       int
		Address         struct {
			ID int
			Address
		}
		NestedAddress *NestedAddress `germ:"embedded;embeddedPrefix:nested_address_"`
		CountryMixin
	}
...
func TestSameForeignKey(t *testing.T) {
	...
	type User struct {
		...
		Name string
		UUID string
		Aux  *UserAux `germ:"foreignkey:UUID;references:UUID"`
	}
...
func TestBelongsToSameForeignKey(t *testing.T) {
	...
	type UserAux struct {
		...
		Aux  string
		UUID string
		User User `germ:"ForeignKey:UUID;references:UUID;belongsTo"`
	}
...
func TestHasOneWithSameForeignKey(t *testing.T) {
	...
	type User struct {
		germ.Model
		Profile      Profile `germ:"ForeignKey:ID;references:ProfileRefer"`
		ProfileRefer int
	}
