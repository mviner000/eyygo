...
type UserIndex struct {
	Name         string `germ:"index"`
	Name2        string `germ:"index:idx_name,unique"`
	Name3        string `germ:"index:,sort:desc,collate:utf8,type:btree,length:10,where:name3 != 'jinzhu'"`
	Name4        string `germ:"uniqueIndex"`
	Name5        int64  `germ:"index:,class:FULLTEXT,comment:hello \\, world,where:age > 10"`
	Name6        int64  `germ:"index:profile,comment:hello \\, world,where:age > 10"`
	Age          int64  `germ:"index:profile,expression:ABS(age),option:WITH PARSER parser_name"`
	OID          int64  `germ:"index:idx_id;index:idx_oid,unique"`
	MemberNumber string `germ:"index:idx_id,priority:1"`
	Name7        string `germ:"index:type"`
	Name8        string `germ:"index:,length:10;index:,collate:utf8"`

	// Composite Index: Flattened structure.
	Data0A string `germ:"index:,composite:comp_id0"`
	Data0B string `germ:"index:,composite:comp_id0"`

	// Composite Index: Nested structure.
	Data1A string `germ:"index:,composite:comp_id1"`
	CompIdxLevel1C

	// Composite Index: Unique and priority.
	Data2A string `germ:"index:,unique,composite:comp_id2,priority:2"`
	CompIdxLevel2C
}

type CompIdxLevel1C struct {
	CompIdxLevel1B
	Data1C string `germ:"index:,composite:comp_id1"`
}

type CompIdxLevel1B struct {
	Data1B string `germ:"index:,composite:comp_id1"`
}

type CompIdxLevel2C struct {
	CompIdxLevel2B
	Data2C string `germ:"index:,unique,composite:comp_id2,priority:1"`
}

type CompIdxLevel2B struct {
	Data2B string `germ:"index:,unique,composite:comp_id2,priority:3"`
}
...
func TestParseIndexWithUniqueIndexAndUnique(t *testing.T) {
	type IndexTest struct {
		FieldA string `germ:"unique;index"` // unique and index
		FieldB string `germ:"unique"`       // unique

		FieldC string `germ:"index:,unique"`     // uniqueIndex
		FieldD string `germ:"uniqueIndex;index"` // uniqueIndex and index

		FieldE1 string `germ:"uniqueIndex:uniq_field_e1_e2"` // mul uniqueIndex
		FieldE2 string `germ:"uniqueIndex:uniq_field_e1_e2"`

		FieldF1 string `germ:"uniqueIndex:uniq_field_f1_f2;index"` // mul uniqueIndex and index
		FieldF2 string `germ:"uniqueIndex:uniq_field_f1_f2;"`

		FieldG string `germ:"unique;uniqueIndex"` // unique and uniqueIndex

		FieldH1 string `germ:"unique;uniqueIndex:uniq_field_h1_h2"` // unique and mul uniqueIndex
		FieldH2 string `germ:"uniqueIndex:uniq_field_h1_h2"`        // unique and mul uniqueIndex
	}
