...
type UserCheck struct {
	Name  string `germ:"check:name_checker,name <> 'jinzhu'"`
	Name2 string `germ:"check:name <> 'jinzhu'"`
	Name3 string `germ:"check:,name <> 'jinzhu'"`
}
...
func TestParseUniqueConstraints(t *testing.T) {
	type UserUnique struct {
		Name1 string `germ:"unique"`
		Name2 string `germ:"uniqueIndex"`
	}
