...
type (
	// DataType germ data type
	DataType string
	// TimeType germ time type
	TimeType int64
)
...
type Field struct {
	...
	// When a column has a (not Mul) UniqueIndex, Migrator always reports its germ.ColumnType is Unique.
...
func (schema *Schema) ParseField(fieldStruct reflect.StructField) *Field {
	var (
		err        error
		tagSetting = ParseTagSetting(fieldStruct.Tag.Get("germ"), ";")
	)
	...
	if isValuer {
		if _, ok := fieldValue.Interface().(GermDataTypeInterface); !ok {
			...
			getRealFieldValue = func(v reflect.Value) {
				...
				if rv.Kind() == reflect.Struct && !rvType.ConvertibleTo(TimeReflectType) {
					for i := 0; i < rvType.NumField(); i++ {
						for key, value := range ParseTagSetting(rvType.Field(i).Tag.Get("germ"), ";") {
	...
	if dataTyper, ok := fieldValue.Interface().(GermDataTypeInterface); ok {
		field.DataType = DataType(dataTyper.GermDataType())
	}
	...
	if field.GERMDataType == "" {
		field.GERMDataType = field.DataType
	}
	...
	if _, ok := field.TagSettings["EMBEDDED"]; ok || (field.GERMDataType != Time && field.GERMDataType != Bytes && !isValuer &&
