...
func Open(dsn string) germ.Dialector {
	return &Dialector{DSN: dsn}
}

func New(config Config) germ.Dialector {
...
func (dialector Dialector) Initialize(db *germ.DB) (err error) {
	...
	return map[string]clause.ClauseBuilder{
		"INSERT": func(c clause.Clause, builder clause.Builder) {
			if insert, ok := c.Expression.(clause.Insert); ok {
				if stmt, ok := builder.(*germ.Statement); ok {
...
func (dialector Dialector) Migrator(db *germ.DB) germ.Migrator {
...
func (dialector Dialector) BindVarTo(writer clause.Writer, stmt *germ.Statement, v interface{}) {
	writer.WriteByte('?')
}
...
func (dialectopr Dialector) SavePoint(tx *germ.DB, name string) error {
	tx.Exec("SAVEPOINT " + name)
	return nil
}

func (dialectopr Dialector) RollbackTo(tx *germ.DB, name string) error {
