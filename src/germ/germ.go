...
// Config germ config
type Config struct {
	// germ perform single create, update, delete operations in transactions by default to ensure database data integrity
...
// Option germ option interface
type Option interface {
	Apply(*Config) error
	AfterInitialize(*DB) error
}

// DB germ DB definition
type DB struct {
	*Config
	Error        error
	RowsAffected int64
...
func (db *DB) SetupJoinTable(model interface{}, field string, joinTable interface{}) error {
	...
	for _, ref := range relation.References {
		...
		f.GERMDataType = ref.ForeignKey.GERMDataType
		if f.Size == 0 {
...
//
//	db.ToSQL(func(tx *germ.DB) *germ.DB {
