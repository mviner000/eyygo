...
var _ germ.Migrator = (*Migrator)(nil)
...
// GermDataTypeInterface Germ data type interface
type GermDataTypeInterface interface {
	GermDBDataType(*germ.DB, *schema.Field) string
}

// RunWithValue run migration with statement value
func (m Migrator) RunWithValue(value interface{}, fc func(*germ.Statement) error) error {
	stmt := &germ.Statement{DB: m.DB}
...
func (m Migrator) DataTypeOf(field *schema.Field) string {
	...
	if dataTyper, ok := fieldValue.Interface().(GermDataTypeInterface); ok {
		if dataType := dataTyper.GermDBDataType(m.DB, field); dataType != "" {
			return dataType
		}
	}
...
func (m Migrator) FullDataTypeOf(field *schema.Field) (expr clause.Expr) {
	...
	if field.HasDefaultValue && (field.DefaultValueInterface != nil || field.DefaultValue != "") {
		if field.DefaultValueInterface != nil {
			defaultStmt := &germ.Statement{Vars: []interface{}{field.DefaultValueInterface}}
...
func (m Migrator) GetQueryAndExecTx() (queryTx, execTx *germ.DB) {
	queryTx = m.DB.Session(&germ.Session{})
	execTx = queryTx
	if m.DB.DryRun {
		queryTx.DryRun = false
		execTx = m.DB.Session(&germ.Session{Logger: &printSQLLogger{Interface: m.DB.Logger}})
	}
...
func (m Migrator) AutoMigrate(values ...interface{}) error {
	for _, value := range m.ReorderModels(values, true) {
		...
		if !queryTx.Migrator().HasTable(value) {
			...
			if err := m.RunWithValue(value, func(stmt *germ.Statement) error {
				...
				for _, dbName := range stmt.Schema.DBNames {
					var foundColumn germ.ColumnType
...
func (m Migrator) CreateTable(values ...interface{}) error {
	for _, value := range m.ReorderModels(values, false) {
		tx := m.DB.Session(&germ.Session{})
		if err := m.RunWithValue(value, func(stmt *germ.Statement) (err error) {
			...
			if tableOption, ok := m.DB.Get("Germ:table_options"); ok {
...
func (m Migrator) DropTable(values ...interface{}) error {
	...
	for i := len(values) - 1; i >= 0; i-- {
		tx := m.DB.Session(&germ.Session{})
		if err := m.RunWithValue(values[i], func(stmt *germ.Statement) error {
...
func (m Migrator) HasTable(value interface{}) bool {
	...
	m.RunWithValue(value, func(stmt *germ.Statement) error {
...
func (m Migrator) AddColumn(value interface{}, name string) error {
	return m.RunWithValue(value, func(stmt *germ.Statement) error {
...
func (m Migrator) MigrateColumn(value interface{}, field *schema.Field, columnType germ.ColumnType) error {
	if field.IgnoreMigration {
		return nil
	}
...
func (m Migrator) MigrateColumnUnique(value interface{}, field *schema.Field, columnType germ.ColumnType) error {
...
// ColumnTypes return columnTypes []germ.ColumnType and execErr error
func (m Migrator) ColumnTypes(value interface{}) ([]germ.ColumnType, error) {
	columnTypes := make([]germ.ColumnType, 0)
	execErr := m.RunWithValue(value, func(stmt *germ.Statement) (err error) {
		rows, err := m.DB.Session(&germ.Session{}).Table(stmt.Table).Limit(1).Rows()
		if err != nil {
			return err
		}
...
// CreateView create view from Query in Germ.ViewOption.
// Query in Germ.ViewOption is a [subquery]
//
//	// CREATE VIEW `user_view` AS SELECT * FROM `users` WHERE age > 20
//	q := DB.Model(&User{}).Where("age > ?", 20)
//	DB.Debug().Migrator().CreateView("user_view", Germ.ViewOption{Query: q})
//
//	// CREATE OR REPLACE VIEW `users_view` AS SELECT * FROM `users` WITH CHECK OPTION
//	q := DB.Model(&User{})
//	DB.Debug().Migrator().CreateView("user_view", Germ.ViewOption{Query: q, Replace: true, CheckOption: "WITH CHECK OPTION"})
//
// [subquery]: https://Germ.io/docs/advanced_query.html#SubQuery
func (m Migrator) CreateView(name string, option germ.ViewOption) error {
	if option.Query == nil {
		return germ.ErrSubQueryRequired
	}
...
func (m Migrator) GuessConstraintAndTable(stmt *germ.Statement, name string) (*schema.Constraint, *schema.CheckConstraint, string) {
...
// nolint:cyclop
func (m Migrator) GuessConstraintInterfaceAndTable(stmt *germ.Statement, name string) (_ schema.ConstraintInterface, table string) {
	if stmt.Schema == nil {
...
func (m Migrator) BuildIndexOptions(opts []schema.IndexOption, stmt *germ.Statement) (results []interface{}) {
...
type BuildIndexOptionsInterface interface {
	BuildIndexOptions([]schema.IndexOption, *germ.Statement) []interface{}
}
...
func (m Migrator) ReorderModels(values []interface{}, autoAdd bool) (results []interface{}) {
	...
	parseDependence = func(value interface{}, addToList bool) {
		dep := Dependency{
			Statement: &germ.Statement{DB: m.DB, Dest: value},
		}
...
func (m Migrator) CurrentTable(stmt *germ.Statement) interface{} {
	if stmt.TableExpr != nil {
...
// GetIndexes return Indexes []germ.Index and execErr error
func (m Migrator) GetIndexes(dst interface{}) ([]germ.Index, error) {
...
// TableType return tableType germ.TableType and execErr error
func (m Migrator) TableType(dst interface{}) (germ.TableType, error) {
	return nil, errors.New("not support")
}
